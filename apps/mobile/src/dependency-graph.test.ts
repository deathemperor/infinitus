 ...
 * the `@infinitus/mobile` package test task).
 */
...
describe("mobile dependency graph", () => {
  ...
  it("keeps upward imports from state/lib/components/native into features at the ceiling", () => {
    ...
    const ceilings: ReadonlyArray<readonly [Layer, Layer, number, string]> = [
      ...
      // The fork adds two: side questions and pin-at-creation under
      // features/infinitus (the ceilings here are the fork's counts, not upstream's).
      ["state", "features", 8, "state must not add imports from features"],
      ...
      ["components", "features", 34, "components must not add imports from features"],
