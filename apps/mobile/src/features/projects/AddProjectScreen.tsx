...
} from "@infinitus/client-runtime/operations/projects";
import {
...
import {
  ...
  isWindowsPlatform,
} from "@infinitus/client-runtime/state/projects";
import {
  CommandId,
  type EnvironmentId,
...
function SectionTitle(props: { readonly children: string }) {
  return (
<<<<<<< HEAD
    <Text className="px-1 text-2xs font-infinitus-bold tracking-[0.7px] uppercase text-foreground-muted">
=======
    <Text
      className={
        Platform.OS === "android"
    ...
    >
      <View className="flex-row items-center gap-3">
        ...
        <View className="flex-1 gap-0.5">
          <Text className="text-base leading-snug font-infinitus-bold">{props.title}</Text>
          {props.subtitle ? (
  ...
  return (
    ...
    >
      {props.loading ? (
        ...
        <Text className="text-base font-infinitus-bold text-primary-foreground">{props.label}</Text>
      )}
    </Pressable>
  );
}
...
function EmptyEnvironmentState() {
  ...
  return (
    <View className="items-center gap-3 rounded-2xl bg-card px-5 py-8">
      <Text className="text-center text-lg font-infinitus-bold">Environment unavailable</Text>
      ...
      >
        <Text className="text-sm font-infinitus-bold text-primary-foreground">Add environment</Text>
      </Pressable>
    </View>
  );
}
    ...
    <AddProjectShell title="Clone destination">
      ...
      {repositoryTitle ? (
        <View className="rounded-[24px] bg-card px-4 py-3">
          <Text className="text-base font-infinitus-bold">{repositoryTitle}</Text>
