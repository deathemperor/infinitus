      ...
      >
        ...
        <View className="min-w-0 flex-1">
          <View className="flex-row items-center gap-2">
            <Text
              className="min-w-0 shrink text-base font-infinitus-medium text-foreground"
            ...
            {props.option.isDefault ? (
              <View className="rounded-md bg-subtle-strong px-1.5 py-0.5">
                <Text className="text-3xs font-infinitus-bold text-foreground-muted">Default</Text>
              </View>
            ) : null}
            {props.option.isLegacy ? (
              <View className="rounded-md bg-subtle px-1.5 py-0.5">
                <Text className="text-3xs font-infinitus-bold text-foreground-muted">Legacy</Text>
              </View>
            ) : null}
...
function ProviderHeader(props: {
  ...
  const content = (
    <>
      ...
      <Text className="text-sm font-infinitus-medium text-foreground-muted">{props.label}</Text>
      {props.collapsible ? (
        <>
          <View className="flex-1" />
          {props.collapsed ? (
            <Text className="text-2xs font-infinitus-medium text-foreground-muted">
              {props.modelCount}
            </Text>
          ) : null}
          <SymbolView
...
function DisclosureRow(props: {
  ...
  return (
    ...
    >
      <Text className="text-sm font-infinitus-medium text-foreground">{props.label}</Text>
      <View className="flex-1" />
      {props.value ? (
...
function ThreadSettingsOptionsItem(props: {
  ...
  return (
    <View style={{ paddingBottom: insets.bottom + bottomToolbarInset + 12 }}>
      <Text className="px-5 pb-2 pt-2 text-sm font-infinitus-medium text-foreground-muted">
        Options
      </Text>
      <Animated.View
      ...
      {Platform.OS !== "ios" && session.hasLegacyModels ? (
        <>
          <Text className="px-5 pb-2 pt-7 text-sm font-infinitus-medium text-foreground-muted">
            Catalog
          </Text>
...
<<<<<<< HEAD
              <View className="flex-row gap-2 pt-3">
                ...
                >
                  <Text className="text-sm font-infinitus-medium text-foreground">All</Text>
                </Pressable>
                <Pressable
                  accessibilityRole="button"
                  accessibilityState={{
                ...
                >
                  <Text className="text-sm font-infinitus-medium text-foreground">Favorites</Text>
                </Pressable>
              </View>
=======
>>>>>>> upstream-sync-7445aa733-upstream-renamed
