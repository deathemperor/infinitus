...
export class PortalCaptureShortcut {
  ...
  async configure() {
    ...
    if (!this.hasSession || this.version < 2)
      throw new Error(
        `Open your desktop's shortcut settings and allow ${PRODUCT_NAME}'s capture shortcut.`,
      );
    await this.call({
  ...
  private failed = (error: unknown) => {
    ...
    this.update({
      ...
      shortcutMessage: this.managedByHyprland
        ? `Couldn't connect to Hyprland shortcuts. Make sure xdg-desktop-portal-hyprland is running, then restart ${PRODUCT_NAME}.`
    ...
    if (this.managedByHyprland) {
      this.update({
        ...
        shortcutMessage: shortcut
          ...
          : `Hyprland did not register the capture action. Check that xdg-desktop-portal-hyprland is running, then restart ${PRODUCT_NAME}.`,
      });
      return;
    }
