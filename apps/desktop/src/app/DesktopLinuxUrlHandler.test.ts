...
describe("DesktopLinuxUrlHandler", () => {
  ...
  it("carries structured context on registration errors", () => {
    ...
    assert.equal(
      writeError.message,
      "Failed to register the infinitus:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
    ...
    assert.equal(
      exitError.message,
      "Failed to register the infinitus:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
