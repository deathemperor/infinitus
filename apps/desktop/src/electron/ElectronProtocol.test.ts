...
describe("ElectronProtocol", () => {
  ...
  it.effect("serves the bundled client from disk without a backend", () =>
    Effect.gen(function* () {
      ...
      const request = (pathname: string, init?: RequestInit) =>
        Effect.promise(() => handler!(new Request(`infinitus://app${pathname}`, init)));
  ...
  it.effect("proxies the stable renderer origin to the current app server", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const response = yield* Effect.promise(() =>
            handler!(
              new Request("infinitus-dev://app/api/health?verbose=1", {
                headers: {
                  accept: "application/json",
                  origin: "infinitus-dev://app",
                  referer: "infinitus-dev://app/",
          ...
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "img-src 'self' infinitus-dev: blob: data: http: https:",
          );
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "font-src 'self' infinitus-dev: data:",
          );
        }),
      );
      ...
      assert.deepEqual(unhandleMock.mock.calls, [["infinitus-dev"]]);
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() => handler!(new Request("infinitus://other/")));
        }),
      );
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() => handler!(new Request("infinitus-dev://app/")));
        }),
      );
  ...
  it.effect("preserves protocol registration failures", () =>
    Effect.gen(function* () {
      ...
      assert.equal(error.scheme, "infinitus-dev");
      assert.strictEqual(error.cause, cause);
      assert.equal(error.message, 'Failed to register Electron protocol scheme "infinitus-dev".');
    }).pipe(Effect.provide(protocolLayer)),
  );
  ...
  it.effect("preserves protocol unregistration failures", () =>
    Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(error.scheme, "infinitus");
        assert.strictEqual(error.cause, cause);
        assert.equal(error.message, 'Failed to unregister Electron protocol scheme "infinitus".');
      }
    }).pipe(Effect.provide(protocolLayer)),
  );
  ...
  it("keeps executable sources host-restricted while allowing runtime network resources", () => {
    ...
    assert.deepEqual(directives["media-src"], ["'self'", "infinitus:", "blob:", "http:", "https:"]);
    assert.deepEqual(directives["frame-src"], ["'self'", "blob:", "http:", "https:"]);
    assert.deepEqual(directives["font-src"], ["'self'", "infinitus:", "data:"]);
  });
});
