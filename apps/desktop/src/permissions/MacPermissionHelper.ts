...
<header>↑ Drag ${PRODUCT_NAME} into the list above</header>
<button id="app" draggable="true" aria-label="Drag ${PRODUCT_NAME} to System Settings, or click to reveal in Finder"><img src="${escapeHtml(icon)}" alt="" draggable="false">${PRODUCT_NAME}</button>
</main></body></html>`;
}
...
export class MacPermissionHelper {
  ...
  async show(
    ...
    if (!appIcon) throw new Error(`The packaged ${PRODUCT_NAME} icon is missing.`);
