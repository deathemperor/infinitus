...
public struct TeamPublisher {
    ...
    /// per transcript chunked and once per batch pushed — never per
    /// chunk: each hop to the main actor commits a CA transaction.
    /// Sealing is not a phase of its own: `TeamClient.publish` seals and
    /// pushes in one call.
    public struct Progress: Equatable, Sendable {
        /// "scan" — a transcript was chunked; "push" — a batch reached the remote.
        public var phase: String
        /// Transcripts chunked so far, of `total` this pass will chunk.
        public var done: Int
    ...
    /// days (Stats v2 `+`) — the `days/` files. Threads and transcripts
    /// come from the desktop (`TeamThreadSources`), not from here.
    ...
    /// at anything. `fleets`/`fleetRows`/`blockers` come from the app's
    /// fleet view; the CLI sends none. `threads`/`live`/`transcripts`
    /// come from Infinitus desktop; `desktop` false says it did not
    /// answer, and the index is then left as it was.
    public struct Sources {
        /// The Mac's name, as `now.json` carries it.
        public var machine: String
        /// A stats scan the caller already has (the app's StatsModel,
        /// which scans on its own cadence; the CLI's own scan). nil
        ...
        /// are the one part of a team dir that grows without bound. Above
        /// this the oldest transcript copies go; a later re-share covers
        /// what is left.
        ...
        /// Checked once per transcript, before chunking it: true flushes
        /// what is staged, saves the cursor and returns the report with
        /// `stopped`. A batch already inside `git push` still finishes —
        /// the per-batch state save is what makes a kill safe.
        public var shouldStop: (@Sendable () -> Bool)?
        public var calendar: Calendar = .current
        public init(home: String, machine: String) { self.home = home; self.machine = machine }
    }
    ...
    public struct Report: Equatable, Encodable {
        ...
        /// Bytes of transcript rows this pass did not reach (a stop cut
        /// them off); the next pass drains them.
    ...
    /// used are the ones corelibs-foundation implements too.
    ...
    /// One transcript's rows past its cursor as redacted JSONL lines, one
    /// `Data` per line (newline included). A row that fails to encode is
    /// dropped: the rest of the thread still travels.
    static func lines(of transcript: TeamThreadSources.Transcript, from offset: Int, redact: (String) -> String) -> [Data] {
        transcript.rows.dropFirst(max(0, offset)).compactMap { row in
            guard let data = try? CanonicalJSON.encode(row) else { return nil }
            return Data((redact(String(decoding: data, as: UTF8.self)) + "\n").utf8)
        }
    }

    /// Lines packed into chunks of at most `maxBytes`; a line above it is
    /// its own chunk (`TeamChunker`'s rule, over rows instead of a file).
    static func pack(_ lines: [Data], maxBytes: Int = TeamChunker.maxChunkBytes) -> [Data] {
        var out: [Data] = []
        var current = Data()
        for line in lines {
            if !current.isEmpty, current.count + line.count > maxBytes { out.append(current); current = Data() }
            current.append(line)
        }
    ...
    public func publish(sources: Sources, now: Date = Date()) throws -> Report {
        ...
        let live = sources.live.filter { !exclusions.excludes(project: $0.project) }
        let toChunk = off(TeamKinds.transcripts) ? [] : sources.transcripts.filter {
            choices.includes($0.threadId) && !exclusions.excludes(project: $0.project)
        }
        var chunked = 0
        ...
        // The index is state, not history: written every pass the desktop
        // answered, so `at` is a truthful "as of". A pass it did not answer
        // leaves the last one rather than publishing an empty list.
        if !off(TeamKinds.threads), sources.desktop {
            try stage(TeamKinds.threads, "threads/index.json",
        ...
        func remaining(from index: Int) -> Int {
            toChunk.dropFirst(index).reduce(0) { total, transcript in
                total + Self.lines(of: transcript, from: state.transcripts[transcript.key]?.offset ?? 0, redact: redact).reduce(0) { $0 + $1.count }
            }
        }

        note("scan")
        for (index, transcript) in toChunk.enumerated() {
            if sources.shouldStop?() == true {
                try flush()
                try state.save(teamDir: teamDir)
                report.stopped = true
                report.remainingBytes = remaining(from: index)
                report.prunedCopies = pruneCopies(cap: sources.copiesCapBytes)
                return report
            }
            try drainingPool {
                let cursor = state.transcripts[transcript.key] ?? TeamPublishState.Cursor()
                var seq = cursor.seq
                // The cursor counts rows, not bytes: the desktop hands the
                // thread whole each time, and a row already published is
                // never sent again.
                for chunk in Self.pack(Self.lines(of: transcript, from: cursor.offset, redact: redact)) {
                    seq += 1
                    let path = transcript.chunkPath(seq: seq)
                ...
                state.transcripts[transcript.key] = TeamPublishState.Cursor(seq: seq, offset: max(cursor.offset, transcript.rows.count))
                // Between transcripts, never inside one: the cursor a flush
    ...
    public func reshare(days: Int, now: Date = Date(), calendar: Calendar = .current) throws -> Report {
        ...
        for path in subpaths {
            ...
            if kind == TeamKinds.transcripts {
                // `path` is copies-relative: transcripts/<thread>/…
                let thread = path.split(separator: "/").dropFirst().first.map(String.init) ?? ""
                guard choices.includes(thread) else { continue }
            }
