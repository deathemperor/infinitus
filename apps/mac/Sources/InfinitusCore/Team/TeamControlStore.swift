...
// Spec §8 lane 2, both sides. The grantor's pass runs after every fetch
// through the SAME Endpoint the HTTP route uses (one replay set, one
// rate limit); the driver reads acks off the reader.
extension TeamControl {
    ...
    public enum Store {
        ...
        public static func grantorPass(client: TeamClient, endpoint: inout Endpoint, handled: inout Handled,
            ...
            // The answers minted after an inline `pending` — a decision, a
            // wait that timed out — go out here, on the same path as the
            // first ack (replaced, so the reader sees one per id). A
            // driver no longer in the roster gets nothing.
