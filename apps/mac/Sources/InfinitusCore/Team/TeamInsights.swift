...
public enum TeamInsights {
    ...
    public static func comparison(_ reader: TeamReader, period: Stats.Period, now: Date = Date(),
        ...
        reader.members.values.map { m in
            MemberRow(kid: m.kid, name: m.name, role: m.role,
                      ...
                      online: isOn(m, now: now), threadsNow: m.now?.live.count ?? 0,
    ...
    public static func isOn(_ m: TeamReader.Member, now: Date) -> Bool {
        ...
        return Int(now.timeIntervalSince1970) - n.at <= onlineWindow && !n.live.isEmpty
    }
    ...
    /// Who works where, from every member's threads index: threads
    /// created inside the period, grouped by project, by spend (the
    /// desktop's usage estimate; a thread without one weighs nothing).
    public static func repos(_ reader: TeamReader, period: Stats.Period, now: Date = Date(),
        ...
        for m in reader.members.values {
            for t in m.threads where t.createdAt >= start {
                var share = byProject[t.project, default: [:]][m.kid] ?? RepoRow.Share(kid: m.kid, name: m.name, usd: 0, turns: 0)
                share.usd += t.usage?.costUsd ?? 0
                share.turns += t.turns
                byProject[t.project, default: [:]][m.kid] = share
            }
        }
        return byProject.map { project, shares in
            let members = shares.values.sorted { $0.usd == $1.usd ? $0.name < $1.name : $0.usd > $1.usd }
            return RepoRow(project: project, usd: members.reduce(0) { $0 + $1.usd }, turns: members.reduce(0) { $0 + $1.turns }, members: members)
        }
    ...
    /// threads waiting on an approval or an answer, crashes today. Stale members are
    ...
    public static func blockers(_ reader: TeamReader, now: Date = Date()) -> [Blocker] {
        ...
        for m in reader.members.values.sorted(by: { $0.name == $1.name ? $0.kid < $1.kid : $0.name < $1.name }) {
            ...
            for t in n.live where t.activityLine?.hasPrefix("Waiting") == true {
                out.append(Blocker(kid: m.kid, name: m.name, kind: "waiting", text: "\(t.title) is waiting for you"))
            }
            if n.crashesToday > 0 {
    ...
    public static func aggregates(_ reader: TeamReader, roster: TeamRoster, period: Stats.Period, now: Date = Date(),
        ...
        return TeamDocs.Aggregates(period: period.rawValue, from: team.from, to: team.to, at: Int(now.timeIntervalSince1970),
                                   ...
                                   hours: hours(rows), repos: repos.map { TeamDocs.Aggregates.Repo(project: $0.project, usd: $0.usd, turns: $0.turns, members: $0.members.count) },
