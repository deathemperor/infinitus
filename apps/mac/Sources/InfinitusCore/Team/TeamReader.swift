...
/// the latest `now.json`, the threads index, crash summaries, and the
/// transcript chunks a thread has (listed here, decrypted on demand).
public struct TeamReader {
    ...
    public static func fold(headers: [(entry: StoreEntry, header: Envelope.Header)], roster: TeamRoster,
        ...
        for (entry, header) in headers {
            ...
            case TeamKinds.threads:
                if let doc = decode(TeamDocs.ThreadsIndex.self, entry.path), doc.schema == 1 { member.threads = doc.threads }
    ...
    /// member's stats, now, threads, fleet and crash docs through one
