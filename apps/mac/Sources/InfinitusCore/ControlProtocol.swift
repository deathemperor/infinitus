...
public struct ControlCommand: Codable, Sendable, Equatable {
    ...
    public static let all: [ControlCommand] = [
        ...
        ControlCommand(name: "status", effect: .read,
                       ...
                       replyShape: "{version, sha, engines:{swapd:{enabled,registered,binaryPath?,daemon?:stopped|running|backingOff|refused|schemaMismatch,error?}, cliproxy:{enabled,registered,keyPresent,error?}, 9router:{enabled,registered,keyPresent,error?}}, badge, signInRunning, playground, forkTunnel:{enabled, port, state, url?, hostname?}, bundlePath, nested}"),
        ...
        ControlCommand(name: "aws-login", args: ["<profile>"], options: ["--local", "--remote", "--status"],
                       effect: .human,
        ...
        ControlCommand(name: "aws-login-code", args: ["<profile>"], effect: .write, stdin: "secret",
                       ...
                       replyShape: "{state}"),
        ControlCommand(name: "gcloud-login", args: ["<account|default|application-default>"], options: ["--local", "--remote", "--status"],
                       effect: .human,
        ...
        ControlCommand(name: "proxy", effect: .read,
                       summary: "CLIProxyAPI settings: base URL, whether a key is stored, whether the engine is on, routing strategy, session affinity (absent on a proxy without the route), the routing caveat and the last error.",
                       replyShape: "{baseURL, keyPresent, enabled, routingStrategy?, sessionAffinity?, caveat?, error?}"),
        ...
        ControlCommand(name: "push", effect: .write, stdin: "payload",
                       summary: "A thread phase change from Infinitus desktop on stdin ({kind: \"thread.phase\", threadId, title, phase, detail?}): pushed as one line through the Mac's channels — Notification Center and the phone — with the Mac's own gating. Title and phase only, never prompt text. Or the phone's lock-screen thread card (#1047): {kind: \"thread.activity\", state} with upstream's AgentActivity props ({title, subtitle, activeCount, updatedAt, activities}) starts or redraws the card on every phone that registered an `agent-activity(-start)` token, `state: null` ends it; no Notification Center line. Either way the reply says what was addressed: `targets` is the number of phones a request went out to and `kinds` the requests per token kind (alert, agent-activity, agent-activity-start) — both 0 when no phone is registered for it or the pusher is not configured.",
                       replyShape: "{pushed, card?, targets, kinds: {<kind>: n}}"),
