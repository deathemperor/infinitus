...
public struct ControlCommand: Codable, Sendable, Equatable {
    ...
    public static let all: [ControlCommand] = [
        ...
        ControlCommand(name: "policy", args: ["<fleet>"], effect: .read, requires: "settings",
                       summary: "The engine's switching-policy knobs for that fleet (swapd `config list`), its own JSON untouched: each key with its value, whether it is set, its default and its help line.",
                       replyShape: "{fleet, settings:[{key, value, isSet, default, help}]}"),
        ControlCommand(name: "policy-set", args: ["<fleet>", "<key>", "<value>"], effect: .write, requires: "settings",
                       summary: "Set one knob by its bare key (`strategy`, `threshold`, `model`); the engine refuses a value off the knob's type or range in its own words, never clamps. Its daemon reads the change on its next tick; nothing restarts.",
                       replyShape: "{fleet, settings:[{key, value, isSet, default, help}]} (the one key)"),
        ControlCommand(name: "policy-unset", args: ["<fleet>", "<key>"], effect: .write, requires: "settings",
                       summary: "Drop one knob so its default applies again.",
                       replyShape: "{fleet, settings:[{key, value, isSet, default, help}]} (the one key)"),
        ...
        ControlCommand(name: "auto-ignite", args: ["<fleet>", "<n>", "on|off"], effect: .write, requires: "autoIgnite",
                       ...
                       replyShape: "{fleet}"),
        ControlCommand(name: "reset", args: ["<fleet>", "<n>"], effect: .write, requires: "reset",
                       summary: "Spend one of account n's banked limit resets (Claude's /reset, swapd reset) as that account; the active account is untouched. Refused when the row carries no `resets` or its `hold` says the next one cannot be spent yet. Takes seconds.",
                       replyShape: "{fleet}"),
        ...
        ControlCommand(name: "signin-begin", args: ["<fleet>"], options: ["--relogin <email>", "--window"],
                       effect: .human, requires: "addOAuth|addToken",
                       summary: "Start this fleet's sign-in with no window on the Mac: the reply carries the OAuth URL for the caller to show (the desktop app, #677). Poll signin-status; hand the code back with signin-code. An engine that takes the redirect itself listens on this Mac's loopback (redirectPort): a caller whose browser is on another machine hands signin-code the address that browser ended on instead of a code. --window shows the Mac's own sign-in sheet instead (passkeys work there; for a caller on this Mac whose browser has no private window), so there is nothing to hand back.",
                       replyShape: "{flowId, url, pasteCode:Bool, redirectPort?:Int, window:Bool, label}"),
        ControlCommand(name: "signin-status", args: ["<flowId>"], effect: .read,
                       ...
                       replyShape: "{flowId, phase: starting|waitingForCode|waitingForToken|registering|done|failed, error?, url?, pasteCode:Bool, redirectPort?:Int, account?}"),
        ControlCommand(name: "signin-code", args: ["<flowId>"], effect: .write, stdin: "secret",
                       summary: "Hand the code from the OAuth success page — or, for a sign-in with a redirectPort, the loopback address the browser ended on — (stdin, never argv) to the waiting sign-in; ok:false carries the CLI's rejection.",
        ...
        ControlCommand(name: "client-activity", options: ["--body <json>"], effect: .write,
                       ...
                       replyShape: "{clientId}"),
        ControlCommand(name: "peer-sync", options: ["--body <json>"], effect: .write,
                       summary: "The desktop's push of every other environment's fleets — the body: {machines:[{id, label, connected, fleets?:[Fleet as `fleets` prints]}], results:[{id, ok, error?}]} — and the popup's answer: the row actions it queued for those machines since the last push, each in that machine's own control words. The desktop runs them there and reports each outcome in `results` on its next push.",
                       replyShape: "{commands:[{id, machine, command, args, options}]}"),
        ...
        ControlCommand(name: "engine-update-check", args: ["swapd"], effect: .read,
                       summary: "The engine's running version against its newest GitHub release for this Mac's platform (#1577): `updatable` when the release is newer; `installed` names a release already put in place by engine-update; `error` the fetch's own words.",
                       replyShape: "{engine, current?, latest?, updatable, installed?, error?}"),
        ControlCommand(name: "engine-update", args: ["swapd"], effect: .restart,
                       summary: "Download the newest release for this platform, check its SHA-256 against the release's, ask the binary its version, install it beside the bundle and relaunch — after which the app and its daemon run the new engine. Refused when nothing newer is published.",
                       replyShape: "{restarting:true, version}"),
        ...
        // reads off this Mac, and the private-project list it honours;
        // everything else Team lives on the relay.
        ControlCommand(name: "team-exclude", args: ["add|remove", "<project slug>"], effect: .write, summary: "Keep a project private (local, never sent).", replyShape: "{projects: [project]}"),
