...
public struct ControlCommand: Codable, Sendable, Equatable {
    ...
    public static let all: [ControlCommand] = [
        ...
        ControlCommand(name: "sessions", effect: .read,
                       summary: "The live Claude Code sessions: pid, name, folder, status, session id, the account alias they run on and start time (ISO 8601).",
                       replyShape: "[{pid, name?, cwd, status?, kind, sessionId, account?, startedAt?}]"),
