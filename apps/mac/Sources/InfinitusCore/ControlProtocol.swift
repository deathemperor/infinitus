...
public struct ControlCommand: Codable, Sendable, Equatable {
    ...
    public static let all: [ControlCommand] = [
        ...
        ControlCommand(name: "team-sessions", args: ["<kid|name>"], effect: .read,
                       summary: "A teammate's live sessions that a grant of theirs lets this Mac drive.",
                       replyShape: "[{id, project, status, name}]"),
        ControlCommand(name: "team-drive",
                       args: ["<kid|name>", "<session|->", "<send|approve|mode|resume|key|stop|resume-past|delete|swap|hold>", "[text…]"],
                       effect: .write,
                       summary: "One command into a teammate's session under their grant: the network lanes first, the store when none answers. `-` for the machine-scoped actions (swap, hold).",
                       replyShape: "{id, lane, outcome, detail}"),
        ControlCommand(name: "team-pending", effect: .read,
                       summary: "Teammates' commands waiting for your Allow (#220 Phase 2).",
                       replyShape: "[{id, driver, session, action, expires}]"),
        ControlCommand(name: "team-allow", args: ["<id>"], effect: .write,
                       summary: "Allow one waiting command; the answer rides the store to the driver.",
                       replyShape: "{id, outcome, detail}"),
        ControlCommand(name: "team-deny", args: ["<id>"], effect: .write,
                       summary: "Deny one waiting command; the answer rides the store to the driver.",
                       replyShape: "{id, outcome, detail}"),
        ControlCommand(name: "team-grants", effect: .read,
                       summary: "This Mac's session-control grants (#220): who may do what to which sessions.",
                       replyShape: "{schema, grants: [{id, audience, sessions, capabilities, preauthorized?, since, expires?}]}"),
        ControlCommand(name: "team-grant", args: ["<leaders|team|kid,…>"],
                       options: ["--cap <a,b>", "--sessions <id,id>", "--pre <a,b>", "--expires <seconds>"], effect: .write,
                       summary: "Add a grant: the capabilities in --cap for the people named on the sessions named (all when --sessions is absent); --pre names the ones that run without asking (delete never does); --expires counts from now.",
                       replyShape: "{id, audience, sessions, capabilities, preauthorized?, since, expires?}"),
        ControlCommand(name: "team-revoke", args: ["<id>"], effect: .write,
                       summary: "Remove one grant; the next command under it is refused noGrant.",
                       replyShape: "{removed}"),
        ...
        ControlCommand(name: "push", effect: .write, stdin: "payload",
                       summary: "A thread phase change from Infinitus desktop on stdin ({kind: \"thread.phase\", threadId, title, phase, detail?, local?, slack?}): pushed as one line through the Mac's channels — Notification Center, the phone, Slack/Telegram — with the Mac's own gating; `local: false` skips the Notification Center notice (the desktop shows its own banner) and still reaches the phone and Slack/Telegram; `slack: false` skips the Slack webhook post alone (a Slack-started thread reports in its own thread there), Telegram and the phone unchanged. Title and phase only, never prompt text. Or the phone's lock-screen thread card (#1047): {kind: \"thread.activity\", state} with upstream's AgentActivity props ({title, subtitle, activeCount, updatedAt, activities}) starts or redraws the card on every phone that registered an `agent-activity(-start)` token, `state: null` ends it; no Notification Center line.",
        ...
        ControlCommand(name: "approve", effect: .read, stdin: "payload",
                       ...
                       replyShape: "{decision}"),
        ControlCommand(name: "permission", effect: .write, stdin: "payload",
                       summary: "A PermissionRequest hook payload on stdin (#79): for a session with remote asks on, registers the ask and answers its id to park on with permission-wait; otherwise {remote: false} and the hook steps aside.",
                       replyShape: "{remote, id?, expiresAt?}"),
        ControlCommand(name: "permission-wait", args: ["<id>"], effect: .read,
                       summary: "Parks until the ask is decided from the desktop or the web, at most 60 s; `ask` means no decision came and the terminal's own prompt takes over — never a silent allow.",
                       replyShape: "{decision: allow|deny|ask, message?}"),
        ControlCommand(name: "permission-decide", args: ["<id>", "<allow|deny>"], options: ["--message <text>"], effect: .write,
                       summary: "Answer an open permission ask; the message rides a deny back to Claude Code.",
                       replyShape: "{decided, decision}"),
        ControlCommand(name: "permission-pending", effect: .read,
                       summary: "The permission asks still open, oldest first: the tool and a bounded rendering of its input (a command, a path, or the input's keys).",
                       replyShape: "[{id, pid?, sessionId, tool, input, askedAt, expiresAt}]"),
        ControlCommand(name: "session-remote", args: ["<pid|name>", "<on|off>"], effect: .write,
                       summary: "Route a running session's permission prompts to the desktop and the web: while on, each prompt waits up to 60 s for an Allow/Deny there before the terminal shows its own. Per session, forgotten with the app.",
                       replyShape: "{pid, remote}"),
        ControlCommand(name: "sessions", effect: .read,
                       ...
                       replyShape: "[{pid, name?, cwd, status?, kind, profile?, permissionMode?, sessionId, account?, startedAt?, needs, remote}]"),
        ...
        ControlCommand(name: "session-delete", args: ["<sessionId>"], options: ["--yes"], effect: .write,
                       summary: "Hide a past session from every list on this Mac; the transcript is never touched.",
                       replyShape: "{hidden}"),
        ...
        ControlCommand(name: "session-stop", args: ["<pid>"], options: ["--yes"], effect: .destructive,
                       summary: "Stop a live session: Esc now, SIGTERM after a 5 s grace if the same session still owns the pid.",
                       replyShape: "{pid, sessionId, esc, grace}"),
