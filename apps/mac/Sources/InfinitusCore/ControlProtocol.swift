...
public struct ControlCommand: Codable, Sendable, Equatable {
    ...
    public static let all: [ControlCommand] = [
        ...
        ControlCommand(name: "lock", args: ["on|off|now|relock", "[immediately|5m|1h|sleep]"], effect: .write,
                       summary: "Biometric lock: `on` runs the unlock prompt on the Mac once and turns the setting on; `off` turns it off; `now` locks the pop-out and Settings; `relock <choice>` sets the re-lock delay.",
