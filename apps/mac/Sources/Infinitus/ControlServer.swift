...
final class ControlServer {
    ...
    private func dispatch(_ r: ControlRequest) async throws -> ControlReply {
        ...
        case "nudge":
            ...
            let hinted = model.sessionStatusHints.withLock { $0.apply(ClaudeSessions.list(claudeDir: claudeDir)) }
            guard let record = hinted.first(where: { Int($0.pid) == pid }) else {
        ...
        case "session-mode":
            guard r.args.count >= 2 else { throw Fail("usage: session-mode <pid|name> <\(SessionModes.hookModes.map(\.mode).joined(separator: "|"))>") }
