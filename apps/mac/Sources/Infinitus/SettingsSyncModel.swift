...
final class SettingsSyncModel: ObservableObject {
    ...
    /// The last line `report` logged, so a failing tick says it once.
    ...
    func tick() async {
        ...
        guard let dir = Self.containerDir() else {
            report("iCloud settings sync: iCloud Drive not available on this Mac")
            return
        }
        ...
        // under a toggle switched off meanwhile (observed as "pushed
        // 00:23" under an off toggle, 2026-08-30).
        ...
        if let remote, remote != lastSeen, remote != local {
            ...
            do {
                ...
                report("iCloud settings sync: push failed — \(error.localizedDescription)")
            }
    ...
    /// A tick that cannot do its job says so once in the event log. The
    /// pane that used to show pulled/pushed/in sync went with the export
    /// and import buttons beside it (user 2026-09-15) — the routine
    /// outcomes are chatter, a failure is not. Repeats are dropped: the
    /// tick runs on every snapshot refresh, and a Mac without iCloud
    /// Drive would otherwise fill the log.
    private func report(_ text: String) {
        guard text != lastReported else { return }
        lastReported = text
        model?.logEvent("other", icon: "exclamationmark.triangle", text)
    }
