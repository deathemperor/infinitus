...
public struct T3Input: View {
    ...
    let leading: Lucide?
    /// A caller-owned focus binding (e.g. a sidebar's hidden ⌘F button) —
    /// `nil` keeps the field's own private `@FocusState`, unchanged for
    /// every existing call site.
    let externalFocus: FocusState<Bool>.Binding?
    public init(text: Binding<String>, placeholder: String, leading: Lucide? = nil, focus: FocusState<Bool>.Binding? = nil) {
        self._text = text; self.placeholder = placeholder; self.leading = leading; self.externalFocus = focus
    }
    public var body: some View {
        let p = t3.web
        let radius = T3Theme.Metrics.radius   // "rounded-lg" = var(--radius)
        let isFocused = externalFocus?.wrappedValue ?? internalFocused
        HStack(spacing: 8) {
            ...
            TextField("", text: $text, prompt: Text(placeholder).foregroundStyle(p.placeholder.color))
                ...
                .focused(externalFocus ?? $internalFocused)
        }
        ...
        .overlay(RoundedRectangle(cornerRadius: radius).stroke(isFocused ? p.ring.color : p.input.color, lineWidth: 1))
        .overlay(RoundedRectangle(cornerRadius: radius + 1.5)
            .stroke(isFocused ? p.ring.color.opacity(0.24) : .clear, lineWidth: 3)
            .padding(-1.5))
    }
}
