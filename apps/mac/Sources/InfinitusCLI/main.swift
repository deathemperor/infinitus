...
var secret: String?
if stdinPiped, ["proxy-key", "9router-password", "aws-login-code", "gcloud-login-code", "signin-code", "aws-login-callback", "push", "desktop-credential", "team-create", "team-join"].contains(command) {
...
// comes from stdin, so `infinitusctl client-activity < report.json` works.
// `activities-token --forget <slot>` carries no body: reading stdin there
// waits on a pipe the caller never closes (#835).
if stdinPiped, ["activities-token", "client-activity"].contains(command), options[ControlBody.option] == nil,
   options["forget"] == nil {
    let data = FileHandle.standardInput.readDataToEndOfFile()
