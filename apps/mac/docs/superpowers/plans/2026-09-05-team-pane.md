...
- No cswap anywhere; never read engine internals (the engine's own backup dir, proxy config/auth files, `~/.9router`); Claude Code's own files under `~/.claude` are fine; never `~/.aws/login` or `~/.aws/sso`.
