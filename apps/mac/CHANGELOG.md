...
### Mac
- An account row is flagged stale only when swapd's last fetch for it actually failed, and the tooltip then names the error; a row that is merely between polls shows its numbers unflagged.
...
### Desktop
- A thread started from Slack no longer gets its milestones posted twice to Slack when the Mac's own Slack webhook is set up; Telegram and the phone are unchanged.
...
- A thread waiting for an approval or an answer, finished or failed can now be pushed through Infinitus to the phone, Slack and Telegram, from a switch on Settings › Infinitus; the Mac skips its own notice for these, the desktop's banner stays the one.
...
- A pass lists the store's branches once for its fetch, grantor pass and reload together, not once each, while the store's refs hold (#499).
...
- The Linux companion answers the timeline and commands routes (and reads a checkpoint ladder), so a paired phone's thread and slash menu work against a Linux session.
...
- The engine's minute polls and repeated "no switch" lines no longer fill the Activity log.
