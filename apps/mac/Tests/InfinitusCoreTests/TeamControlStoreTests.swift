...
final class TeamControlStoreTests: XCTestCase {
    ...
    /// Leader = grantor, Bob = driver; a grant to Bob on thread t1 for send.
    func team() throws -> (grantor: TeamClient, driver: TeamClient, grantorDir: URL) {
        ...
        _ = grants.add(audience: .members([bob.identity.kid]), threads: .some(["t1"]), capabilities: [TeamGrants.send], now: 1_030)
    ...
    func endpoint(_ client: TeamClient, dir: URL, now: Int, executed: @escaping (String, String?) -> Void) -> TeamControl.Endpoint {
        TeamControl.Endpoint(identity: client.identity, roster: { client.roster?.doc }, grants: { TeamGrants.load(teamDir: dir) },
                             threads: { ["t1"] },
                             execute: { c in executed(c.action, c.text); return TeamControl.Reply(outcome: "delivered") },
    ...
    func testStoreLaneRoundTripsAndAcksOnce() throws {
        ...
        let now = 2_000
        let cmd = TeamControl.Command(id: "c-0123456789", to: grantor.identity.kid, thread: "t1", action: TeamGrants.send, text: "hello", at: now)
        let d = TeamControl.Deliver(http: { _, _, _, _, _ in XCTFail("no endpoints, no dial"); return (0, Data()) })
    ...
    func testRefusedStoreCommandIsAckedWithTheRefusal() throws {
        let (grantor, driver, dir) = try team()
        let now = 2_000
        let cmd = TeamControl.Command(id: "c-0123456780", to: grantor.identity.kid, thread: "t1", action: TeamGrants.interrupt, text: nil, at: now)
    ...
    func testAPendingStoreCommandIsAnsweredTwiceOnOnePath() throws {
        ...
        grants.add(audience: .members([driver.identity.kid]), threads: .some(["t1"]), capabilities: [TeamGrants.interrupt], now: 1_031)
        try grants.save(teamDir: dir)
        let now = 2_000
        let cmd = TeamControl.Command(id: "c-p123456789", to: grantor.identity.kid, thread: "t1", action: TeamGrants.interrupt, text: nil, at: now)
        _ = try TeamControl.Drive.store(cmd, client: driver)
        _ = try grantor.fetch()
        var executed: [(String, String?)] = []
        var ep = endpoint(grantor, dir: dir, now: now + 10) { executed.append(($0, $1)) }
        ...
        XCTAssertEqual(executed.map(\.0), ["interrupt"])
