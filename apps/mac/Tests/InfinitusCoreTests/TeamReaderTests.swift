...
final class TeamReaderTests: XCTestCase {
    ...
    func testFoldGroupsPerMemberSumsDaysAndOrdersChunks() throws {
        ...
        docs["m/\(a.kid)/now.json"] = try CanonicalJSON.encode(TeamDocs.Now(at: 50, machine: "a", live: [], fleets: [], blockers: ["aws"], crashesToday: 0, sharesTo: [:], desktop: true))
        docs["m/\(a.kid)/crashes.json"] = try CanonicalJSON.encode(TeamDocs.Crashes(crashes: ["Mac · crash · x"]))
        docs["m/\(a.kid)/threads/index.json"] = try CanonicalJSON.encode(TeamDocs.ThreadsIndex(at: 50, threads: [TeamDocs.ThreadRow(id: "t1", title: "Fix", project: "app", status: "idle", createdAt: 1, updatedAt: 2, turns: 1)], fleets: []))
        ...
        let headers = [
            ...
            entry("m/\(a.kid)/threads/index.json", kind: "threads", from: a.kid, at: 50),
        ...
        XCTAssertEqual(reader.members[a.kid]?.threads.map(\.id), ["t1"])
        ...
        XCTAssertEqual(reader.members[a.kid]?.kinds, ["stats", "now", "crashes", "threads", "transcripts"])
    ...
    func testCreateCodeRequestApprovePublishFetchReadThenRemove() throws {
        ...
        // publish (two members, same day) → fetch → read. Stats come from
        // a scan of each member's transcript tree; threads and their
        // transcripts from what the desktop said (a fixture here).
        func sources(_ name: String, home: String) throws -> TeamPublisher.Sources {
            var s = TeamPublisher.Sources(home: home, machine: name)
            s.entries = StatsScanner.scan(projectsDir: try writeProjects(scratch, name: name), cacheURL: nil,
                                          calendar: .current, maxAge: 10_000 * 86_400).entries
            // The fixture is stamped 2026-09-04; the window is widened so
            // the test does not go red the day the floor passes it.
            s.historyDays = 10_000
            s.desktop = true
            s.threads = [TeamDocs.ThreadRow(id: "t1", title: "Fix the app", project: "app", status: "idle", createdAt: 1, updatedAt: 2, turns: 1)]
            s.transcripts = [TeamThreadSources.Transcript(threadId: "t1", project: "app", rows: [
                .init(role: "user", text: "use sk-ant-api03-abcdefghijklmnopqrstuvwxyz please", at: 1),
                .init(role: "assistant", text: "sure", at: 2),
            ])]
            return s
        }
        let s = try sources("alice", home: "/Users/alice")
        _ = try TeamPublisher(client: alice, paths: ap).publish(sources: s)
        let sb = try sources("bob", home: "/home/bob")
        ...
        XCTAssertEqual(reader.members[alice.identity.kid]?.threads.map(\.project), ["app"])
        // The transcript chunk is listed, fetched on demand, and redacted.
        let chunks = try XCTUnwrap(reader.members[alice.identity.kid]?.transcripts["t1"])
        XCTAssertEqual(chunks, ["t/\(alice.identity.kid)/transcripts/t1/1.jsonl"])
        try leader.fetchTranscripts(from: alice.identity.kid, session: "t1")
        let lines = String(decoding: try leader.read(chunks[0]).1, as: UTF8.self).split(separator: "\n").map(String.init)
        let rows = try lines.map { try CanonicalJSON.decode(TeamThreadSources.Row.self, from: Data($0.utf8)) }
        XCTAssertEqual(rows.map(\.text), ["use [redacted-key] please", "sure"])
        XCTAssertNil(reader.members[alice.identity.kid]?.transcripts["nope"])
