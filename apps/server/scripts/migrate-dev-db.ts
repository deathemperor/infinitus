 ...
 * ~/.infinitus database, then run this checkout's migrations against it.
 *
...
export class MigrateDevDbSharedHomeError extends Schema.TaggedError<MigrateDevDbSharedHomeError>()(
  ...
  override get message(): string {
    return "Refusing to rebuild the shared ~/.infinitus database. Use an isolated --base-dir.";
  }
}
...
export interface RunMigrateDevDbInput {
  ...
  /** Source database. Defaults to `~/.infinitus/userdata/state.sqlite`. */
...
export const migrateDevDbCommand = Command.make(
  ...
  Command.withDescription(
    "Rebuild the worktree dev database from a pruned snapshot of the real ~/.infinitus data, then run migrations.",
  ),
);
