...
const version = "1.2.3";
const archiveName = `infinitus-${version}-linux-x64.tar.gz`;
  ...
  ProcessRunner.ProcessRunner.of({
    run: (input) =>
      Effect.gen(function* () {
        ...
        yield* fs
          .writeFileString(path.join(stagingDir, "infinitus"), "#!/bin/sh\n")
          .pipe(Effect.orDie);
        return {
          stdout: "",
          stderr: "",
...
it.layer(NodeServices.layer)("ensurePinnedRuntimeInstalled", (it) => {
  it.effect("installs the verified release archive as the runtime executable", () =>
    Effect.gen(function* () {
      ...
      assert.equal(paths.entryPath, path.join(paths.versionDir, "infinitus"));
  ...
  it.effect("refuses an archive whose checksum does not match the release", () =>
    Effect.gen(function* () {
      ...
      assert.equal(error.step, "verifying the release archive checksum");
