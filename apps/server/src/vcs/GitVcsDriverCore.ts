      ...
      : resolveProjectFileBackedSetting(
          ...
          options?.submodules != null
            ...
            : yield* Effect.gen(function* () {
                // The first project file that reads decides, in the server's order.
                for (const fileName of T3_PROJECT_FILE_NAMES) {
                  const contents = yield* fileSystem
                    .readFileString(path.join(worktreePath, fileName))
                    .pipe(Effect.option);
                  if (Option.isNone(contents)) continue;
                  const file = parseT3ProjectFile(contents.value);
                  if (file === null) {
                    yield* Effect.logWarning(
                      `${fileName} is invalid; initializing submodules recursively`,
                      { worktreePath },
                    );
                  }
                  return file;
