    ...
    Effect.gen(function* () {
      ...
      > {
        ...
        const runtime = yield* makeAntigravityAcpRuntime({
          ...
          spawn: buildAntigravityAcpSpawnInput({
            ...
            profile,
            cwd: input.cwd,
            baseEnv: withProviderSessionEnvironment(processEnvironment, input),
            auth,
      ...
      const makeDisposableRuntime = Effect.fn("AntigravityDriver.makeDisposableRuntime")(function* (
        ...
        const runtime = yield* makeRuntime({
          cwd,
          clientInfo: { name: "infinitus-provider-setup", version: "0.0.0" },
          mcpServers: [],
      ...
      const textGeneration = yield* makeAntigravityTextGeneration({
        ...
        makeRuntime: (cwd) =>
          makeRuntime({
            cwd,
            clientInfo: { name: "infinitus-text", version: "0.0.0" },
            mcpServers: [],
          }),
      });
