        ...
        Effect.mapError(
          (cause) =>
            new EffectAcpErrors.AcpTransportError({
              detail: "Failed to process Oh My Pi ACP event.",
              cause,
            }),
        ),
      );
            ...
            yield* acp.handleRequestPermission((params) =>
              mapExtensionFailure(
                Effect.gen(function* () {
                  ...
                  // Only full access auto-approves. Oh My Pi never prompts for a
                  // plain edit — it gates `edit` solely when the payload rewrites
                  // to a delete or a move — and it stamps `kind` on bash alone, so
                  // an auto-accept-edits branch here could only ever fire on the
                  // destructive rewrites the mode is meant to keep asking about.
        ...
        return yield* Effect.gen(function* () {
          ...
          if (input.attachments && input.attachments.length > 0) {
            for (const attachment of input.attachments) {
              // Oh My Pi ACP advertises image prompt capability. Generic files
              // reach the agent through the path line ProviderService puts in
              // the prompt.
          ...
          // Oh My Pi joins every text block with a blank line and hands the whole
          // string to its slash-command parser, which reads everything after the
          // command name as arguments (`/compact <instructions>`). A second block
          // would therefore become the compaction's focus text, so a turn that is
          // only a slash command is sent bare.
          const result = yield* ctx.acp
            .prompt({
              prompt: isBareSlashCommand(rawPrompt)
                ? promptParts
                : [
                    ...promptParts,
                    {
                      type: "text",
                      text: buildRuntimeInstructions({ harness: "Oh My Pi", model: resolvedModel }),
                    },
                  ],
            })
            .pipe(
