...
const serviceInstallCommand = Command.make("install", serviceReconcileFlags).pipe(
  Command.withDescription(`Install ${PRODUCT_NAME} as a background service for this user.`),
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription(`Stop and remove the ${PRODUCT_NAME} background service.`),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription(`Show whether the ${PRODUCT_NAME} background service is installed.`),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
