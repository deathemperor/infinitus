...
describe("createEnvironmentThreadStateAtoms", () => {
  ...
  // Fork (#897): while the completion marker is awaited the replay folds and
  // reaches the state once, at `synchronized`, however the events are batched.
  it.effect.each([1, 16, 500])(
    "publishes a replay once, at synchronized (batch size: %i)",
    (batchSize) =>
      Effect.gen(function* () {
        const h = yield* makeHarness();
        const unmount = h.registry.mount(h.stateAtom);
