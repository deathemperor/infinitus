...
/// `T3ThreadSettled` and `T3ThreadStatus` reason over.
extension T3Thread {
    ...
    /// (the record init above keeps `record.sessionId`). The phone migrates to
    /// this init after B-1 (its current `T3HomeList` mapping uses `String(pid)`
    /// / raw cwd ids); until then the two clients key threads differently.
    ...
    public init(session: SessionDetail, facts: SessionFacts?, progress: SessionProgress?,
        ...
        let turnAt = facts?.latestTurn.map { $0.completedAt ?? $0.startedAt ?? $0.requestedAt }
        let updatedAt = [progress?.lastActivityAt, turnAt, createdAt].compactMap { $0 }.max() ?? createdAt
        ...
        self.init(id: "pid:\(session.pid)",
                  ...
                  hasActionableProposedPlan: false,
                  latestUserMessageAt: facts?.latestUserMessageAt, latestTurn: turn,
                  // computed updatedAt, not createdAt (plan): equal when progress is nil, and the
                  // later signal is what the settled/raise-hand comparison wants.
