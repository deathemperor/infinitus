...
public struct ControlCommand: Codable, Sendable, Equatable {
    ...
    public static let all: [ControlCommand] = [
        ...
        ControlCommand(name: "gcloud-login", args: ["<account|default|application-default>"], options: ["--pid <session pid>", "--local", "--remote", "--status"],
                       effect: .human,
                       summary: "Run `gcloud auth login --no-launch-browser` (or `auth application-default login` for application-default) and report the URL to open on another device; the page ends with a verification code pasted back via `gcloud-login-code`. --local opens this Mac's browser instead; --status only reports the login and starts nothing. Watch `aws-logins` for the state (provider \"gcloud\").",
