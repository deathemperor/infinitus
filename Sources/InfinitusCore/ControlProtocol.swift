...
public struct ControlCommand: Codable, Sendable, Equatable {
    ...
    public static let all: [ControlCommand] = [
        ...
        ControlCommand(name: "fleets", effect: .read,
                       summary: "Every fleet with accounts, usage, active/next, the engine's capabilities and, while priority_mode is on, its headroom verdict (#616: absent = mode off or no usage seen yet; a transient usage gap keeps the last verdict).",
        ...
        ControlCommand(name: "team-join", args: ["<your name>"], effect: .write, stdin: "secret",
                       ...
                       replyShape: "team-status"),
        ControlCommand(name: "team-hostname", options: ["--zone <zone>", "--label <label>"], effect: .write, stdin: "secret",
                       summary: "Settings › Team › Hostnames: keep the Cloudflare zone and label member hostnames are minted under, with the API token on stdin (checked against the zone before it is kept).",
