...
public struct ControlCommand: Codable, Sendable, Equatable {
    ...
    public static let all: [ControlCommand] = [
        ...
        ControlCommand(name: "team-drive", args: ["<kid|name>", "<session>", "<send|approve|mode|resume|key>", "[text…]"], effect: .write,
                       summary: "One command into a teammate's session under their grant: the network lanes first, the store when none answers.",
