...
public struct ControlCommand: Codable, Sendable, Equatable {
    ...
    public static let all: [ControlCommand] = [
        ...
        ControlCommand(name: "show", args: ["popout|settings|wall|workspace [sidebar|thread|composer|draft|switcher]|session <pid|name>"], effect: .write,
                       summary: "Open a window: the pinned pop-out, Settings, the wall (toggle), the workspace (optionally on a screen), or one session's chat window.",
                       replyShape: "{shown}"),
        ...
        ControlCommand(name: "activities-token", options: ["--body <json>"], effect: .write,
                       summary: "Register a phone's push token — the body `POST /activities/token` takes: {kind, token, deviceId, deviceName, environment, themeID?, macId?}. Without --body the CLI reads the JSON from stdin.",
                       replyShape: "{slot}"),
        ...
        ControlCommand(name: "hide", args: ["popout|settings|workspace"], effect: .write,
                       summary: "Close the pinned pop-out (the e2e no-lease window), Settings, or the workspace.",
                       replyShape: "{hidden}"),
