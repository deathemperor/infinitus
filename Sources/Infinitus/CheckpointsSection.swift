...
/// session, see its per-prompt checkpoints newest first, Restore one
/// behind a confirm. Nothing is read until the disclosure opens; every
        ...
        DisclosureGroup(isExpanded: $expanded) {
            VStack(alignment: .leading, spacing: 4) {
                if sessions.isEmpty {
                    ...
                    ForEach(checkpoints.reversed(), id: \.n) { c in row(c) }
                }
                if let note {
