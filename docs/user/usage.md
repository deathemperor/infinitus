...
databases, including Infinitus-managed profiles. Set `OPENCODE_DATA_DIR` or `ANTIGRAVITY_DATA_DIR` on the
...
This includes headless Infinitus sessions and desktop usage across machines; the same account counts
once across connected environments. Without an accessible CLI login, Infinitus shows a
notice instead of incomplete local totals. Infinitus does not estimate missing tokens from conversation text.
On macOS, choose **Enable Cursor usage** on Usage to allow Infinitus to read your existing CLI login
...
the environment. Infinitus cannot report limits for external OpenCode servers because their credentials
