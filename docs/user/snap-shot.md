...
Shift keys together. Infinitus attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while Infinitus is in front captures Infinitus itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. Infinitus refuses shortcuts
...
terminal contents. If an app is slow to answer, Infinitus attaches the screenshot without the data
rather than waiting.
...
shortcut does not fire, check Infinitus under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. Infinitus shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. Infinitus shows the exact binding it will
...
**Other Wayland desktops.** Infinitus uses the desktop's screenshot portal when it can capture the
