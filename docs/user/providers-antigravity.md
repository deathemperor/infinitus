...
Infinitus runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for Infinitus to confirm
...
hostname. Only that Infinitus sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. Infinitus's separate Plan mode
...
Infinitus keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. Infinitus also reads `.gemini/skills` and the
