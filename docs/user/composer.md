...
On Claude threads, the composer shows how long the prompt cache stays warm after the last turn.
A green border and a timer by the send button mean your next message reads the conversation from
cache; amber means it expires within a few minutes. Once it expires, the border returns to normal
and the timer reads "expired": the next message sends the whole context again at full cost, so a
new thread may be cheaper if you do not need the history. Claude Code keeps some sessions' cache
for an hour and others for five minutes; the timer follows what each turn reported. On mobile, the
same timer is in the thread menu's **Thread usage**.
