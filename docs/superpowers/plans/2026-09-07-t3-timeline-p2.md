...
Live smoke: not possible on a dev instance — mock mode runs no mirror listener and a live one would collide with the bundle's port (`tools/e2e.sh:388`). After merge and Infi's rebuild, against the bundle (port + pairing token from the app):
...
Expected: pin → 200 with `"pinnedAt"`; snooze without until → 400; snapshot shows `factsByPid` entries. Then `infinitusctl perf` twice, 15 s apart, with the pop-out open: idle CPU near 0%. Report on #223.
