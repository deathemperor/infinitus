...
- Produces:
  ...
  public final class AttentionStore: @unchecked Sendable {
      public struct Entry: Codable, Sendable, Equatable {
          public var settledOverride: Bool?; public var settledAt: Date?
          public var snoozedUntil: Date?;    public var pinnedAt: Date?
          public init(settledOverride: Bool? = nil, settledAt: Date? = nil, snoozedUntil: Date? = nil, pinnedAt: Date? = nil)
...
The `record(...)` call — add after `births: self.sessionBirths`:

```swift
                                            facts: { [timelineCache, attentionStore, ownedBox] records in
...
Next to `mirrorServer.sessionInput.set`:

```swift
        mirrorServer.attention.set { [timelineCache, attentionStore, ownedBox] pid, request in
...
If `ownedBox` is a `let` on AppModel it captures fine; if the closure needs `[weak self]` instead, use `self?.ownedBox.existing`.
...
### Task 8: CHANGELOG, PR, notify

**Files:**
- Modify: `CHANGELOG.md` under `## Unreleased` → `### Sessions` (top of the list)

- [ ] **Step 1: CHANGELOG** (one line)

```
- Sessions carry T3-style facts — pending prompts, plan progress, settle/snooze/pin — and the phone can settle, snooze or pin one over the mirror (#223).
```
...
git commit -m "changelog: session facts and attention (#223 P2)
