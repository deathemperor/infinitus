...
- `apps/mobile/src/lib/attachmentUpload.ts` (+ test) — `uploadFileBytes` uploads on the `foreground` session and fails a transfer after `ATTACHMENT_UPLOAD_TIMEOUT_MS`: the default iOS background session replayed a 7.7 MB image the relay cut at two minutes, and the one-at-a-time outbox held every queued reply behind it (2026-09-19). Offered upstream as well; drop this bullet once their copy carries it.
