...
- `apps/desktop/src/infinitus/previewOAuthUserAgent.ts` (+ test), `signInUserAgent.ts` — the browser preview sends Chromium's own user agent to `accounts.google.com` and to no other host: Google answers Electron's native agent with its embedded-browser flow and a preview page's sign-in pop-up ends on "400 … malformed", while a session-wide rewrite breaks Cloudflare Turnstile (#5002). A request header via the session's one `onBeforeSendHeaders` listener, registered from `preview/BrowserSession.ts`; `signInUserAgent` moved out of `InfinitusSignIn.ts` (which re-exports it) so the preview shares it without the sign-in window's service graph. Add a host to `PREVIEW_OAUTH_URLS` only with evidence it refuses the native agent.
