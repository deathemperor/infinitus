...
- `apps/mac/Sources/InfinitusCore/Engines/Swapd/SwapdUpdate.swift` (+ `SwapdUpdateTests`), `apps/web/src/components/settings/infinitus/engineUpdate.logic.ts` (+ test), the `InfinitusEngineUpdateRow` in `InfinitusEnginesPanel.tsx` — an engine release installed from the Engines page (#1577): the Mac's `engine-update-check swapd` reads `releases/latest` of `deathemperor/swapd` and `engine-update swapd` (a restart verb) downloads the platform tarball, checks its SHA-256 against the release's `.sha256`, asks the unpacked binary its version and puts it under `Application Support/Infinitus/engines/swapd-update/` with a `VERSION` sidecar. `SwapdLocator` takes that copy ahead of the bundle only while its sidecar is newer than `InfinitusSwapdVersion`, the version `make-app.sh` stamps into Info.plist from the bundled engine (`EngineVersion.compare`), so the #1530 rule holds: a release whose bundle catches up outranks the copy again and it is skipped like a PATH one. The launch agent needs nothing new: it copies whichever binary the locator chose into `engines/swapd`, as it always did. Contract `InfinitusEngineUpdateCheck`.
...
- `apps/web/src/routes/accounts.tsx`, `apps/web/src/components/accounts/` — the Accounts page (one `EnvironmentAccounts.tsx` group per connected machine) and its Sign-ins section (`SignInsSection.tsx`, `signIns.logic.ts`; models in `packages/client-runtime/src/state/infinitusAccounts.ts`, `infinitusPageState` #693); the provider tabs and marks (`ProviderTabs.tsx`, `providerMark.ts`, `providerAccountCounts`), the Quota windows timeline (`QuotaTimeline.tsx`, model `packages/client-runtime/src/state/infinitusQuotaTimeline.ts`, rolled windows off `utilization --days 1`); Add account / Sign in again (`addAccount.logic.ts`, #671, #1213), the desktop's in-app sign-in (`apps/desktop/src/infinitus/InfinitusSignIn.ts`, `signIn.logic.ts`, #677) and the paste-code path over `infinitus.secret` (#747 step 2). Rules and traps: `docs/internals/accounts-page.md`.
...
- `apps/desktop/src/infinitus/InfinitusSignInRedirect.ts` (+ test) — the engine's loopback listener stood in for on this machine, for a sign-in on another Mac's environment: binds the port, opens the page, hands the redirect's address back to the page for `signin-code`; methods `listenInfinitusSignInRedirect` / `stopInfinitusSignInRedirect`, contracts `InfinitusSignInRedirectListenInput` / `-Result`. Rules and traps: `docs/internals/accounts-page.md`.
...
- `.github/workflows/upstream-sync.yml` — the daily upstream merge as a PR (INFINITUS.md); once `scripts/infinitus-rename.ts --check` passes on `main` it renames upstream's tip on its own branch before merging (#1368 C). A conflicted `pnpm-lock.yaml` is regenerated from upstream's side rather than left to hand-merge, and each new sync closes older still-untouched sync PRs, since its merge contains theirs.
